...
class BeikeNet():
    ...
    def print_infos(self, infos):
        ...
        if infos['mode'] == '4' or infos['mode'] == '12':  # v6模式
            ...
            print(BKNet_UEDV4.format(
                infos['v4_flow'] // 1024 // 1024, infos['v4_flow'] // 1024 % 1024))
            print(BKNet_UEDV6.format(
                infos['v6_flow'] // 1024 // 1024, infos['v6_flow'] // 1024 % 1024))
    ...
    def infos(self):
        ...
        self.WebHeader['Cookie'] = 'myusername={0}; username={0}'.format(
            self.config['username'])
        html = self.light_get(
            url=self.UstbUrls['login'], headers=self.WebHeader)
    ...
    def login(self):
        ...
        self.WebHeader['Cookie'] = 'myusername={0}; username={0}'.format(
            self.config['username'])
        html = self.light_post(
            url=self.UstbUrls['login'], post_data=login_data, headers=self.WebHeader)
    ...
    def logout(self):
        ...
        self.WebHeader['Cookie'] = 'myusername={0}; username={0}'.format(
            self.config['username'])
        html = self.light_get(
            url=self.UstbUrls['logout'], headers=self.WebHeader)
    ...
    def ipv6_conn_test(self, ipv6_addr):
        ipv6_addr_temp = self.get_ip(
            socket.AF_INET6, self.UstbUrls['gg_server'])
